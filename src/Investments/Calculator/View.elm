  ...
  Html.form
    ...
    [ ul
        ...
        [ li
            ...
            , text "$"
            , input [ on "input" targetValue (Signal.message address << (parseFloat SetPrincipal)), value (toString model.principal), id "principal", type' "number", Html.Attributes.min "0", step "500" ] []
            ]
        , li
            []
            [ label [ for "starting-monthly-contribution" ] [ text "Starting Monthly Contribution" ]
            , text "$"
            , input [ on "input" targetValue (Signal.message address << (parseFloat SetStartingMonthlyContribution)), value (toString model.startingMontlyContribution), id "starting-monthly-contribution", type' "number", Html.Attributes.min "0", Html.Attributes.max "5000", step "50" ] []
            ]
        , li
            []
            [ label [ for "investment-increase" ] [ text "Annual Investment Increase" ]
            , input [ on "input" targetValue (Signal.message address << (parseFloat SetAnnualInvestmentIncrease)), value (toString <| model.annualInvestmentIncrease * 100), id "investment-increase", type' "number", Html.Attributes.min "0", Html.Attributes.max "20", step "0.25" ] []
            , text "%"
            ]
        , li
            []
            [ label [ for "annual-rate-of-return" ] [ text "Annual Rate of Return" ]
            , input [ on "input" targetValue (Signal.message address << (parseFloat SetAnnualRateOfReturn)), value (toString <| model.annualRateOfReturn * 100), id "annual-rate-of-return", type' "number", Html.Attributes.min "0", Html.Attributes.max "20", step "0.25" ] []
            , text "%"
            ]
        , li
            []
